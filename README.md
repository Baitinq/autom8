...
Review results are saved to `.autom8/internal/reviews/<task-id>.json` (ephemeral, not committed). Use `autom8 describe <task-id>` to view the results, including issues found and their severity.
...
- `.autom8/internal/` - Ephemeral data (gitignored)
  ...
  - `reviews/` - Review task results as JSON (transient)
